    ...
    var body: some View {
        ScrollView {
            ...
            VStack (alignment: .leading)  {
                Text("Im Folgenden werden, wenn von der 7-Tage App (auch App) die Rede ist, sowohl die 7-Tage App selbst als auch das dazugehörige Widget bezeichnet. Alle Angaben in der App und im Widget sind ohne Gewähr.\n\n").font(.system(size: CGFloat(fontSizeWelcomeScreen)))
                
                Text("Was macht diese App?\n\nDie App zeigt Ihnen anhand des aktuellen Standortes oder anhand einer Suche den jeweiligen 7-Tage Wert an. Der Standort oder der gesuchte Ort können nur in Deutschland liegen. Bei einer Suche nach einem Ort werden mit Hilfe von Reverse Geocoding Koordinaten aus dem angegebenen Ort bestimmt. Die Koordinaten werden verwendet, um den jeweiligen 7-Tage Wert zu bestimmen. Die Angabe des 7-Tage Wertes erfolgt ohne Gewähr.\n\n").font(.system(size: CGFloat(fontSizeWelcomeScreen)))
                
                Text("Benötigt die App eine Standortfreigabe?\n\nFalls Sie den aktuellen 7-Tage Wert an Ihrem aktuellen Ort wissen möchten, muss eine Standortfreigabe erfolgen. Anderweitig kann die App auch genutzt werden, um den 7-Tage Wert an einem anderen beliebigen Ort in Deutschland anzuzeigen.\n\n").font(.system(size: CGFloat(fontSizeWelcomeScreen)))
                ...
                Text("Automatische Datenerhebung\n\nWenn Sie diese App nutzen, werden durch die Benutzung des Internet einige Daten automatisch übermittelt bzw. ausgewertet und gespeichert. Die Auswertung, Übermittlung und Speicherung der Daten ist notwendig, um die jeweiligen Inhalte in der App darstellen zu können. Log-Dateien werden von dieser App nicht erstellt. Bei Benutzung der App werden folgende Daten ausgewertet bzw. übermittelt: Standort (GPS-Koordinaten) des jeweiligen Ortes (falls vom Nutzer freigegeben), gerundet auf drei Nachkommastellen, http-Status-Code der Anfrage (z.B. um eine erfolgreiche oder fehlgeschlagene Abfrage zu prüfen).\nDiese Daten werden erhoben, um die Funktionen in dieser App bereitstellen zu können. Die Rechtsgrundlage hierfür ist Art 6 Abs 1 DS-GVO. Solange diese App die oben beschriebenen Daten nutzt, um die Funktionen und Inhalte dieser App bereitstellen zu können, ist der Nutzer verpflichtet, diese Daten zur Verfügung zu stellen. Dieser Vorgang erfolgt in der Regel automatisch, z.B. durch die Eingabe eines Ortes oder durch die Bereitstellung der Standortdaten. Die App speichert keine Log-Dateien. Werden die oben beschriebenen allgemeinen Daten genutzt, werden keine Rückschlüsse auf die betroffene Person bzw. auf den Nutzer gezogen.\n\n").font(.system(size: CGFloat(fontSizeWelcomeScreen)))
                
                Text("Standortfreigabe\n\nDie App kann entweder mit oder ohne Standortfreigabe genutzt werden. Ist die Standortfreigabe aktiv, so werden die aktuellen Koordinaten genutzt, um den 7-Tage Wert einzuholen. Ohne Standortfreigabe können nur manuell eingegebene Orte nach ihrem 7-Tage Wert durchsucht werden.\nGPS-Koordinaten werden immer nur auf drei Nachkommastellen genau verwendet, um den 7-Tages-Wert zu erhalten.\nFalls Sie in der App die Standortfreigabe erlaubt haben, so werden die Standortdaten zu den folgenden Zwecken verwendet: Informationsanzeige für den aktuellen Standort, ortsbezogene Updates des 7-Tage Wertes. Die Rechtsgrundlage für die Verarbeitung des Standortes ist Art 6 Abs 1 DS-GVO.\nDie Einstellungen zur Standortfreigabe können Sie jederzeit beliebig auf Ihrem Gerät ändern. Die Standortdaten werden nicht gespeichert oder anderweitig ausgewertet.\n\n").font(.system(size: CGFloat(fontSizeWelcomeScreen)))
                
                Text("Werden Daten getrackt oder gespeichert?\n\nDie App trackt während der Nutzung der App keine Nutzerdaten – es erfolgt daher kein Tracking und auch keine Auswertung der Daten. Daten werden nie an Dritte gesendet oder gespeichert – ausgenommen hiervon ist der Standort bzw. die Standort-Koordinaten, die in die http-Anfrage miteinfließen, um den aktuellen 7-Tage Wert anzuzeigen. Die GPS-Koordinaten sind damit Teil der http-Anfrage bzw. des Links, der aufgerufen wird. Sie werden damit nicht direkt an Dritte gesendet.").font(.system(size: CGFloat(fontSizeWelcomeScreen)))
            }
